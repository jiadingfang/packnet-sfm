...
from packnet_sfm.networks.layers.resnet.gray_resnet_encoder import ResnetEncoder
from packnet_sfm.networks.layers.resnet.depth_decoder import DepthDecoder
from packnet_sfm.networks.layers.resnet.eucm_decoder import EUCMDecoder
from packnet_sfm.networks.layers.resnet.layers import disp_to_depth

########################################################################################################################

class EUROCEUCMResNet(nn.Module):
    """
    ...
    def __init__(self, version=None, **kwargs):
        ...
        self.intrinsic_decoder = EUCMDecoder(num_ch_enc=self.encoder.num_ch_enc)
    ...
    def forward(self, rgb):
        ...
        if self.training:
            return [self.scale_inv_depth(d)[0] for d in disps], k
        else:
            return self.scale_inv_depth(disps[0])[0], k
