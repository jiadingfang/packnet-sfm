...
class EUCMMultiViewPhotometricLoss(LossBase):
    """
...
########################################################################################################################
    ...
    def warp_ref_image(self, inv_depths, ref_image, I, ref_I, pose):
        ...
        for i in range(self.n):
            ...
            cams.append(EUCMCamera(I=I).to(device))
            ref_cams.append(EUCMCamera(I=ref_I, Tcw=pose).to(device))
        # View synthesis
...
########################################################################################################################
    ...
    def calc_pixel_mask(self, image, I):
        """
        Calculates mask for valid pixels in EUCM model according equation (23) in the double sphere paper
        ...
        beta = I[:, 5].unsqueeze(1).unsqueeze(2)
        ...
        mask = (r_square <= 1 / beta / (2 * alpha - 1)) | (alpha <= 1/2)
