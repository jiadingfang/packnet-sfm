...
# from packnet_sfm.geometry.camera import 
...
from packnet_sfm.losses.features.autoencoder import Encoder
...
class UCMMultiViewFeatureMetricLoss(LossBase):
    """
    Parameters
    ----------
    num_scales : int
    ...
    def __init__(self, num_scales=4, ssim_loss_weight=0.85, occ_reg_weight=0.1, smooth_loss_weight=0.1,
                 C1=1e-4, C2=9e-4, photometric_reduce_op='mean', featuremetric_reduce_op='mean', disp_norm=True, clip_loss=0.5,
...
########################################################################################################################
    ...
    def warp_ref_image(self, inv_depths, ref_image, I, ref_I, pose):
        ...
        for i in range(self.n):
            ...
            # cams.append(UCMCamera(K=K.float()).scaled(scale_factor).to(device))
            # ref_cams.append(UCMCamera(K=ref_K.float(), Tcw=pose).scaled(scale_factor).to(device))
...
########################################################################################################################
    ...
    def compute_featuremetric_loss(self, ref_warped, images):
        def robust_l1(pred, target):
            eps = 1e-3
            return torch.sqrt(torch.pow(target - pred, 2) + eps ** 2)
        f_ests = [self.extractor(t_est) for t_est in ref_warped]
        f_srcs = [self.extractor(img) for img in images]
        # print('f_ests')
        # print(len(f_ests[0]))
        # print(f_ests[0][0].shape)
        # print(f_ests[0][1].shape)
        # print(f_ests[0][2].shape)
        # print(f_ests[0][3].shape)
        # print(f_ests[0][4].shape)
        # print('f_srcs')
        # print(len(f_srcs[0]))
        # print(f_srcs[0][0].shape)
        loss = [robust_l1(f_ests[i][0], f_srcs[i][0]).mean(1, True) for i in range(self.n)]
        return loss

    def reduce_featuremetric_loss(self, featuremetric_losses):
        # Reduce function
        def reduce_function(losses):
            if self.featuremetric_reduce_op == 'mean':
                return sum([l.mean() for l in losses]) / len(losses)
            elif self.featuremetric_reduce_op == 'min':
                return torch.cat(losses, 1).min(1, True)[0].mean()
            else:
                raise NotImplementedError(
                    'Unknown featuremetric_reduce_op: {}'.format(self.featuremetric_reduce_op))
        # Reduce photometric loss
...
########################################################################################################################
    ...
    def forward(self, image, context, inv_depths,
        ...
        featuremetric_losses = [[] for _ in range(self.n)]
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            featuremetric_loss = self.compute_featuremetric_loss(ref_warped, images)
            for i in range(self.n):
                photometric_losses[i].append(photometric_loss[i])
                featuremetric_losses[i].append(featuremetric_loss[i])
            # If using automask
            if self.automask_loss:
                # Calculate and store unwarped image loss
                ref_images = match_scales(ref_image, inv_depths, self.n)
                unwarped_image_loss = self.calc_photometric_loss(ref_images, images)
                unwarped_featuremetric_loss = self.compute_featuremetric_loss(ref_images, images)
                for i in range(self.n):
                    photometric_losses[i].append(unwarped_image_loss[i])
                    featuremetric_losses[i].append(unwarped_featuremetric_loss[i])
        # Calculate reduced photometric loss
        loss = self.reduce_photometric_loss(photometric_losses)
        # Calculate reduced feature-metric loss
        loss += self.reduce_featuremetric_loss(featuremetric_losses)
