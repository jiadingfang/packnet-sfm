...
########################################################################################################################
    ...
    def reconstruct(self, depth, frame='w'):
        ...
        fx, fy, cx, cy, w = self.fx, self.fy, self.cx, self.cy, self.w
        ...
        mx = (u - cx) / fx
        my = (v - cy) / fy
        r_d = torch.sqrt(mx ** 2 + my ** 2)
        
        x = mx * torch.sin(r_d * w) / (2 * r_d * torch.tan(w / 2))
        y = my * torch.sin(r_d * w) / (2 * r_d * torch.tan(w / 2))
        z = torch.cos(r_d * w)
        z = z.clamp(min=1e-5)
    ...
    def project(self, X, frame='w'):
        ...
        r_u = torch.sqrt(x ** 2 + y ** 2)
        r_d = torch.atan2(2 * r_u * torch.tan(w / 2), z) / w
        
        Xnorm = fx * r_d * x / r_u + cx
        Xnorm = 2 * Xnorm / (W) - 1
        Ynorm = fy * r_d * y / r_u + cy
        Ynorm = 2 * Ynorm / (H) - 1
